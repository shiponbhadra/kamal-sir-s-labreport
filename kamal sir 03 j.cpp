/* CSE department wants to manage a Fast-food shop in its premise. Every student can make pre-order of his/her breakfast before 10 PM of the previous day.
A sales person can manage the data and sells the preordered item to the students.If a student pre-ordered before but not take his/her breakfast and
the sales person can inform it to the department.If he will not be illegible to pre-order the breakfast another time.
...
public:
    ...
    view_student_info()
    {
        ...
        cout<<"Students Birthday: "<<this->date.date<<endl;
    }
    friend orderfood();
    friend blacklist();
};
...
public:
    int seller_no;
    Sellsman() {};
    Sellsman(string name,string date,int seller_no): Person(name,date)
    {
    ...
    view_sellsman()
    {
        ...
        cout<<"Birthday: "<<this->date.date<<endl;
    }
    friend orderfood();
};
class Login
...
int main()
{
    ...
    {
        new Student("Tipu","3-5-1966",department,"11708001"),
        new Student("Sumi","23-3-1948",department,"11708002"),
        new Student("Foysal","2-10-1996",department,"11708003"),
        new Student("Utpol","25-7-1987",department,"11708004")
    };

    Sellsman sells_person("Hassan","1-2-2001",1);
    sells_person.view_sellsman();
    Login *login_data[2]=
    {
        new Login("Rahul","11708005"),
        new Login("Kamrul","11708006")
    };
    Faculty *faculty_member[2]=
    {
        new Faculty("Professor","Dipu Ahmed",department,login_data[0]),
        new Faculty("Lecturer","Dipa Bonik",department,login_data[1])
    };
    for(int i=0; i<3; i++)
